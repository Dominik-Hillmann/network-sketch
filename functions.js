// contains connection points for the lines and the methods they need
var pointContainer =
{
   container : []
};

// draws line between two points in pointContainer.container if they have less than a certain distance to each other
pointContainer.display = function()
{
   for(var i = 0; i < this.container.length; i++)
   {
      for(var j = 0; j < this.container.length; j++)
      {
         ...
         {
            line(this.container[i].x, this.container[i].y, this.container[j].x, this.container[j].y);
         }
      }
   }
}
